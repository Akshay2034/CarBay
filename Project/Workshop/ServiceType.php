...
<?php 
	 ...
	 if($_GET['ed'])
      {
 			$SelQuery="select * from  tbl_servicetype  where servicetype_id='".$id."'";
	...
	if(isset($_POST['btnsubmit']))
	{
		...
		if($id!="")
		{
			$updQuery="update  tbl_servicetype  set servicetype_name='".$d_name."' where servicetype_id='".$id."'";
		...
		{
 			$insQuery= "insert into  tbl_servicetype (servicetype_name)values('$d_name')";
 			//echo $insQuery;
			mysql_query($insQuery,$con) or die(mysql_error());
 			echo "<b>Values Added successfully!";
	    	echo "<meta http-equiv=Refresh content=6;url=ServiceType.php>"; 
		}
	}
	 ...
	 if($_GET['del'])
      {
 		$delQuery="delete from  tbl_servicetype  where servicetype_id='".$id."'";
...
<form id="form1" name="form1" method="post" action="">
  ...
  <br /><br />
    ...
    <?php
   		 	$Selquery="select * from  tbl_servicetype";
     ...
     <tr>
       ...
       <td width="85">
          <a href="ServiceType.php?del=1&amp;delid=<?php echo $row['servicetype_id'];?>">Delete</a>
          <a href="ServiceType.php?ed=1&amp;edid=<?php echo $row['servicetype_id'];?>">Edit</a>
          </td>
